...
class ProductRow extends React.Component{
    ...
    removeFromDatabase(idToRemove){
      ...
      console.log(json)
      fetch("https://moticaserver.onrender.com/remove/product", {    
        method: "delete",
        headers: {
    ...
    editProduct = async(e) => {
      ...
      await fetch("https://moticaserver.onrender.com/updateProduct", {    
        method: "post",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      return (
        <div className="col">
              ...
              <div className="card">
                  ...
                  <div class="modal fade" id={"Producto"+this.props.idObject} tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              ...
                              <label>Nombre:{this.props.name}</label><br/>
                              <input onChange={this.changeName}
                              ></input><br/>
                              <label>Precio:{this.props.price}</label><br/>
                              <input onChange={this.changePrice}
                              ></input><br/>
                              <label>Categoria:{this.props.category}</label><br/>
                              <input onChange={this.changeCategory}
                              ></input><br/>
                              <label>Cantidad:{this.props.cantidad}</label><br/>
                              <input onChange={this.changeQuantity}
                              ></input><br/>
                              <label>Descripcion:{this.props.description}</label><br/>
                            ...
                            <div class="modal-footer">
                              ...
                              <button onClick={this.editProduct} type="button" data-bs-dismiss="modal" class="btn btn-primary">Save changes</button>
                            </div>
                          </div>
                        </div>
                      </div>

                  <img src={this.props.imgSrc} className="card-img-top" alt="..." width="75" height="150"/>
                  <div className="card-body">
                      ...
                      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target={"#Producto"+this.props.idObject}>Editar</button><span>  </span>
                      <button onClick={this.removeProduct} class="btn btn-danger" type="button">Remover</button>
                     </div>
                  </div>
...
function Gestion(){
    ...
    let url = "https://moticaserver.onrender.com/products";
    ...
    for (let i = 0; i < array1.length; i++) {
        ...
        console.log(datos[0])
        array2.push(<ProductRow imgSrc={datos[1]} name={datos[5]} price={datos[6]} description={datos[3]} category={datos[7]} activo={"activo"} idObject={datos[0]} cantidad={datos[2]}/>)
    }
    setProducts(array2)
    ...
    async function submitProduct(e){
        ...
        if (!form.cantidad.length) {
          ...
          await fetch("https://moticaserver.onrender.com/register/product", {    
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
    ...
    return (
        <div className="position-relative" style = {{minHeight: "100%"}}>
            ...
            <form style = {{float: "none"}}>
                <div className=''>
                    <label className='text-white'>Nombre del producto</label><br/>
                    <input value={form.nombre}
                    onChange={(e) => updateForm({ nombre: e.target.value })}
                    ></input><br/>
                    <label className='text-white'>Categoria del producto</label><br/>
                    <input value={form.categoria}
                    onChange={(e) => updateForm({ categoria: e.target.value })}></input><br/>
                    <label className='text-white'>Descripcion del producto</label><br/>
                    <input value={form.descripcion}
                    onChange={(e) => updateForm({ descripcion: e.target.value })}></input><br/>
                    <label className='text-white'>Precio del producto</label><br/>
                    <input value={form.precio}
                    onChange={(e) => updateForm({ precio: e.target.value })}></input><br/>
                    <label className='text-white'>Url de la imagen asociada al producto</label><br/>
                    <input
                    value={form.imgSrc}
                    onChange={(e) => updateForm({ imgSrc: e.target.value })}></input><br/>
                    <label className='text-white'>Cantidad en Stock del producto</label><br/>
                    <input
                    value={form.cantidad}
                    onChange={(e) => updateForm({ cantidad: e.target.value })}></input><br/>
                </div><br/>
                <div className="row row-cols-1 row-cols-md-2 row-cols-md-3 row-cols-md-4 row-cols-md-5 g-4">
                <ProductRow/>
                <ProductRow/>
                <ProductRow imgSrc={form.imgSrc} name={form.nombre} price={form.precio} description={form.descripcion} category={form.categoria} activo="activo"/>
                <ProductRow/>
                <ProductRow/>
                </div>
            </form>
            ...
            <button onClick={submitProduct} type="submit" class="btn btn-success">Agregar producto</button><br/>
            <br></br>
            ...
            <div style = {{minHeight: "100%"}} className="m-4">
                ...
                <div className="row row-cols-1 row-cols-md-2 row-cols-md-3 row-cols-md-4 row-cols-md-5 g-4">
                {products.filter(product => product.props.name.includes(search)).map(filteredProduct => (
