...
class CatalogComponent extends React.Component{
  ...
  render(){
    ...
    return (
      ...
      {this.props.editable ?
      <td  style  = {{width:"14%",minWidth:"15%", maxWidth:"15%"}} class = "col">
        
      <button style  = {{display:"inline"}} onClick = {this.removeOneItem}className="btn btn-secondary">-</button>
          <li style  = {{display:"inline"}} class = "text-white">--</li>
      
     {this.props.cantidad} 
     
    <li style  = {{display:"inline"}} class = "text-white">--</li>
        <button style  = {{display:"inline"}} onClick = {this.addOneItem}className="btn btn-secondary">+</button>
  </td>
        :
        <td >
...
async function addToCart(subtotal){
  ...
  let user = localStorage.getItem(localStorage.getItem("ipAdress")) || false;
   ...
   for(let i = 0 ; i < products.length; i++){
    ...
    listaItems.push([Object(products[i].id), products[i].name, products[i].cantidad, products[i].price, categoria])
   }
   


  let url = "https://moticaserver.onrender.com/numFactura";
  ...
  let numFactura = array1[0].numFactura
   ...
   let newOrder2 = {user:userobj._id, numFactura, subtotal, total, listaItems}
   ...
   await fetch("https://moticaserver.onrender.com/register/order2", {    
     method: "POST",
     headers: {
  ...
  for(let i = 0 ; i <this.state.productsA.length; i++){
    ...
    for(let j = 0 ; j < this.state.productsA[i].cantidad;j++){
      if (this.state.productsA[i].descuento > 0) {
        let descuento = this.state.productsA[i].price * (this.state.productsA[i].descuento/100)
    ...
    k.push(<CatalogComponent editable= {!this.state.cartConfirmed} sumTotal = {sum} functionAlter = {this.change} cantidad = {datos.cantidad} id = {datos.id} imgSrc={datos.imgSrc} name={datos.name} price={datos.price} description={datos.description} category={datos.category} descuento={datos.descuento}/>)
...
confirmOrder = ()=>{
  addToCart(this.state.totalSum).then(alert("Tu orden se ha registrado correctamente", 
  this.setState({cartConfirmed:!this.state.cartConfirmed}), localStorage.removeItem("Products")))
 ...
 render() {
    ...
    <tbody>
      ...
      <div>
        {this.state.cartConfirmed ?
          <div>
           ...
           <button style ={{display:"inline"}} onClick ={this.authorizeCart} className=" text-dark btn btn-secondary">Continuar editando +</button>
           <li class = "text-dark" style ={{display:"inline"}}>  --</li>
          <button style ={{display:"inline"}}  onClick ={this.confirmOrder} className=" text-dark btn btn-secondary">Confirmar orden de compra definitiva</button>
      ...
      <div>
          ...
          <button onClick ={this.authorizeCart} className=" text-dark btn btn-secondary">Confirmar productos</button>
      ...
      <></>
        ...
        <div style  = {{minHeight:"100%"}} className= "position-relative">
          <br></br>
          <h2 class = "text-white">Ups.. parece que no tienes productos añadidos en el carrito</h2>
          <br></br>
          <a className=" btn bg-secondary text-white" href="/Catalogo"> Ir al catálogo</a>
          </div>
        }
