...
function Navigates(){
 ...
 function getUserD(){    
  ...
  if(localStorage.getItem("ipAdress")!=null){            
      let json = JSON.parse(localStorage.getItem(localStorage.getItem("ipAdress")));  
  ...
  constructor(props){
   ...
   if (res.ok) {
    ...
    let data  = getUserD();
        ...
        await fetch("https://moticaserver.onrender.com/update/user", {    
        method: "post",
        headers: {
    ...
    return (
      ...
      <img src={this. state.imgSrc} width="400" height={"300"} alt="Profile"/>
        ...
        <h4 style = {{display:"inline"}}>Nombre:    </h4>
          ...
          <input onChange={(e) => this.updateForm({ name: e.target.value })} defaultValue={this. state.name} style = {{display:"inline"}}></input>
          <br></br>
          <br></br>
          <h4 style = {{display:"inline"}}>Apellido:    </h4>
          <input  onChange={(e) => this.updateForm({ apellido: e.target.value })} defaultValue={this. state.apellido} style = {{display:"inline"}}></input>
          <br></br>
          <br></br>
          <br/>
         ...
         <select onChange={(e) => this.updateForm({ sexo: e.target.value })} defaultValue={this. state.sexo}  className="select" 
           >
        <option value="F" >Femenino</option>
         <option value="M">Masculino</option>             
          ...
          <input onChange={(e) => this.updateForm({ fechaNacimiento: e.target.value })} defaultValue={this.formatDate(this. state.fechaNacimiento)}  type='date'  className=""
          ...
          <input  onChange={(e) => this.updateForm({ cedula: e.target.value })} defaultValue={this. state.cedula} style = {{display:"inline"}}></input>
          <br></br>
          <br></br>
   ...
   function getPerson() {
    getUser().then(currentUser=> {
      ...
      if(currentUser.sexo==="F"){
        perfil.push(<ProfileComponent imgSrc={perfilMujer}  name={currentUser.nombre} apellido={currentUser.apellido} email={currentUser.correo} sexo={currentUser.sexo} rol={currentUser.rol} fechaNacimiento={currentUser.fecha_nacimiento} cedula={currentUser.cedula}/>)
      }else{
        perfil.push(<ProfileComponent imgSrc={perfilHombre}  name={currentUser.nombre} apellido={currentUser.apellido} email={currentUser.correo} sexo={currentUser.sexo} rol={currentUser.rol} fechaNacimiento={currentUser.fecha_nacimiento} cedula={currentUser.cedula}/>)
      }
...
function EditProfile() {
  return (
   <Plantilla sectionToDisplay = {ProfileBuild()}></Plantilla>
  );
}
