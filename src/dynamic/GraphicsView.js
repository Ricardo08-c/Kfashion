...
class ReporteC extends React.Component{
    constructor(props){
      ...
      mapOrdersByDate=(canceladas)=>{
        ...
        for(let i = 0 ; i< this.state.labels.length; i ++){
            ...
            for(let j = 0 ; j< this.state.orders.length; j ++){
                ...
                if(new Date(this.state.orders[j].fecha).toLocaleDateString()== this.state.labels[i]){
                    if(canceladas){
      ...
      getLabels= ()=>{
        ...
        return getDatesBetween(new Date(this.state.fechaini),new Date(this.state.fechafin), true)
  ...
  displayDatesStart=(e)=>{
    ...
    this.forceUpdate() 
      ...
      render(){
        ...
        this.state.map.push(<Orders oneUser ={false} cancel = {false} dataToDisplay = {this.state.ordersToDisplay}/>)
        ...
        return(
            ...
            <div style={{marginLeft:"10%",maxWidth:"80%"}}>
                ...
                <div className="  form-group">
                    <label style  = {{display:"inline"}}className= "text-white"htmlFor="name">Fecha de Inicio </label>
                    <input style={{maxWidth:"15%",display:"inline"}} type='date'  className="form-control"
                    id="name"
                    
                    onChange={this.displayDatesStart} class="form-control" />
                    <li style = {{display:"inline",opacity:"0"}}>-----</li>
                    <label style  = {{display:"inline"}}className= "text-white"htmlFor="name">Fecha Fin </label>
                    <input style={{maxWidth:"15%",display:"inline"}} type='date'  className="form-control"
                    id="name"
                    
                    onChange={this.displayDates} class="form-control" />
                </div>
            ...
            :<></>
                ...
                <div style = {{left: "10%"}}>
                    ...
                    {this.state.ordersToDisplay.length>0? 
...
function Reporte() {
  return (
   <Plantilla sectionToDisplay = {<ReporteC/>}></Plantilla>
  );
}
