...
function Ordersreturn(){
  return (
    <Plantilla sectionToDisplay = {<Orders  dataToDisplay = {false}oneUser ={true}></Orders>}></Plantilla>
  )
}
...
class Orders extends React.Component{
  ...
  componentDidUpdate(){
    if(this.props.dataToDisplay && this.props.dataToDisplay !=this.state.orders){
  ...
  orders = ()=>{
    ...
    for (let i = 0 ; i < this.state.orders.length; i ++)  {
      ...
      <Order estado = {this.state.orders[i].estado}cancel = {this.props.cancel}objUser = {this.state.orders[i].objUser[0]} numFactura={this.state.orders[i].numFactura} index = {i+1}productsA = {prods} fecha = {date.toLocaleDateString()} date = {date}hora = {date.toLocaleTimeString()}dia = {date.getDate()} mes= {date.getMonth()}/>) 
  ...
  takeOrdersUser = async()=>{
    let url = "https://moticaserver.onrender.com/orders";
    let res = await fetch(url);
    let userId = JSON.parse(localStorage.getItem(localStorage.getItem("ipAdress")));
    if (res.ok) {
  ...
  render(){
    ...
    <div style  = {{minHeight:"86%"}} className= " position-relative">
      ...
      <br></br>
      <h5 style = {{display:"inline",left:"10%"}}class = "text-white">Filtrar  </h5>
      <select style ={{display:"inline",maxWidth:"15%"}} value={this.state.selects} onChange={this.setSelects} className="form-select">
      ...
      ||
      dateDif( order.props.date,new Date())<=5 &&  this.state.selects=="Últimos 5 días"
    ||
      dateDif( order.props.date,new Date())<=30   &&  this.state.selects=="Últimos 30 días"
          || this.state.selects === 'Todas' 
      )).map((filteredOrder => 
        <div>
            <div >
            <h5 class = "text-white">{"Orden #"+(filteredOrder.props.index)}</h5>    
            <h5 class = "text-white">{"Numero de factura: "+filteredOrder.props.numFactura}</h5>
            <h5 class = "text-white">{"Estado: "+filteredOrder.props.estado}</h5>
            {!this.props.oneUser?
            <div>
              
               <h5 class = "text-white">{"Usuario que realiza la orden: "+filteredOrder.props.objUser.nombre + " "+filteredOrder.props.objUser.apellido}</h5>
            ...
            <h5  class = "text-white">{"Fecha en que se realizó la orden:  "+(filteredOrder.props.fecha)+",   a las "+(filteredOrder.props.hora)}</h5>   
...
class Order extends React.Component{
    ...
    for(let i = 0 ; i <this.props.productsA.length; i++){
      ...
      for(let j = 0 ; j < this.props.productsA[i].cantidad;j++){
        if (this.props.productsA[i].descuento > 0) {
          let descuento = this.props.productsA[i].price * (this.props.productsA[i].descuento/100)
      ...
      k.push(<CatalogComponent editable= {false} sumTotal = {sum}  cantidad = {datos.cantidad} id = {datos.id} imgSrc={datos.imgSrc} name={datos.nombre} price={precio} description={datos.description} category={datos.category} descuento={datos.descuento}/>)
  ...
  cancelOnMoticaSide=async ()=>{
    ...
    let json = JSON.stringify({numFactura:this.state.numFactura, estado:"Cancelada",motivo:this.state.motivo})
    
    await fetch("https://moticaserver.onrender.com/update/order", {    
      method: "post",
      headers: {
  ...
  <thead class="thead-dark">
    <tr>
      ...
      <th class = "text-white" scope="col">Precio unitario</th>
      <th class = "text-white" scope="col">Cantidad</th>
      <th class = "text-white" scope="col">Descuento</th>
      <th class = "text-white" scope="col">Total de precios unitarios</th>
      
    </tr>
  </thead>

  <tbody>
  {this.productsToCart().map((data) => (
...
data
        ...
        <td >
        <button onClick = {this.cancelarOrden}className="btn btn-secondary">Cancelar Orden</button>
     ...
     <td></td>
      ...
      ?
        ...
        <td >
        <input onChange = {(e)=>{this.setState({motivo:e.target.value})}}class = "input" type={"text"} placeholder='Motivo' ></input>
